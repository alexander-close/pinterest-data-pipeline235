...
Next I create my DAG in the file <UserId>_dag.py.  This defines the DAG <UserId>_dag, configured to run the above Job (via its job ID) daily at 00:00:00. Uploading this dag to the correct AWS S3 bucket, locating it in the Airflow UI, and manually triggering it reesults in a successful run (after a few modifications)!  The DAG code is also pushed to Github.
